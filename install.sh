...
# - start_ros2docker -> build_run.py
# - stop_ros2docker  -> stop.py
#
# Usage:
#   ./install.sh
#
...
#   START_NAME=start_ros2docker   # command name to create
#   STOP_NAME=stop_ros2docker     # command name to create

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$SCRIPT_DIR"

BIN_DIR="${BIN_DIR:-"$HOME/.local/bin"}"
START_NAME="${START_NAME:-start_ros2docker}"
STOP_NAME="${STOP_NAME:-stop_ros2docker}"

START_TARGET="$ROOT_DIR/build_run.py"
STOP_TARGET="$ROOT_DIR/stop.py"
